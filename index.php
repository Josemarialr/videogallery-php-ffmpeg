...
// Cargar categorías y dividirlas (primeras 5 y el resto)
$todas_categorias = $pdo->query("SELECT * FROM categorias ORDER BY nombre ASC")->fetchAll();
$categorias_principales = array_slice($todas_categorias, 0, 5);
$categorias_extra = array_slice($todas_categorias, 5);
    ...
    <div class="row align-items-center mb-4 g-3">
        ...
        <div class="col-lg-8">
            <div class="d-flex flex-wrap gap-1 align-items-center">
                ...
                <!-- Botón Desplegable (+) con Categorías Extras -->
                <?php if (!empty($categorias_extra)): ?>
                    <?php
                        $es_extra_activa = false;
                        foreach ($categorias_extra as $cat_e) {
                            if ($active_cat == $cat_e['id']) {
                                $es_extra_activa = true;
                                break;
                            }
                        }
                    ?>
                    <div class="dropdown position-relative d-inline-block">
                        <button class="btn btn-sm dropdown-toggle <?= $es_extra_activa ? 'btn-primary' : 'btn-outline-secondary text-white' ?>"
                                type="button"
                                id="btnToggleCatExtra">
                            +
                        </button>
                        <ul class="dropdown-menu dropdown-menu-dark shadow" id="menuCatExtra">
                            <?php foreach ($categorias_extra as $cat): ?>
                                <?php
                                    $activa = ($active_cat == $cat['id']);
                                    $url_cat = "index.php?cat=" . $cat['id'] . ($search_query ? "&q=" . urlencode($search_query) : "");
                                ?>
                                <li>
                                    <a class="dropdown-item <?= $activa ? 'active' : '' ?>" href="<?= $url_cat ?>">
                                        <?= htmlspecialchars($cat['nombre']) ?>
                                    </a>
                                </li>
                            <?php endforeach; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    // Control manual para el botón desplegable de categorías extras (+)
    const btnToggle = document.getElementById('btnToggleCatExtra');
    const menuDropdown = document.getElementById('menuCatExtra');

    if (btnToggle && menuDropdown) {
        ...
        document.addEventListener('click', function(e) {
            if (!menuDropdown.contains(e.target) && e.target !== btnToggle) {
                menuDropdown.classList.remove('show');
            }
        });
    }

    // Animación GIF al pasar el mouse por las miniaturas
