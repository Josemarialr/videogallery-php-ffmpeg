...
// Soportar tanto tag (nombre) como tag_id (ID numérico)
$active_tag_id = $_GET['tag_id'] ?? null;
...
// Filtro por Tag (Prioridad a tag_id si existe)
if (!empty($active_tag_id)) {
    $joins[] = "JOIN video_tags vt ON v.id = vt.video_id";
    $where[] = "vt.tag_id = ?";
    $params[] = intval($active_tag_id);

    // Obtener el nombre del tag para mostrar en la interfaz
    $stmt_tag_name = $pdo->prepare("SELECT nombre FROM tags WHERE id = ?");
    $stmt_tag_name->execute([$active_tag_id]);
    $tag_nombre_actual = $stmt_tag_name->fetchColumn() ?: "Tag #{$active_tag_id}";
} elseif (!empty($active_tag)) {
    $joins[] = "JOIN video_tags vt ON v.id = vt.video_id JOIN tags t ON vt.tag_id = t.id";
    $where[] = "t.nombre = ?";
    $params[] = trim($active_tag);
    $tag_nombre_actual = $active_tag;
} else {
    $tag_nombre_actual = null;
}
...
// Cargar meses disponibles agrupados
$sql_fechas = "SELECT
...
function obtenerUrlPagina($num_pagina) {
    ...
    return 'videos_por_tag.php?' . http_build_query($params);
}
...
<div class="container my-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4>
            <?php if ($tag_nombre_actual): ?>
                Videos etiquetados con <span class="text-warning">#<?= htmlspecialchars($tag_nombre_actual) ?></span>
            <?php else: ?>
                Todos los videos por Tag
            <?php endif; ?>
        </h4>
        <a href="tags.php" class="btn btn-outline-light btn-sm">&laquo; Volver a Tags</a>
    </div>
    ...
    <div class="row align-items-center mb-3 g-3">
            ...
            <div class="d-flex flex-wrap gap-1 align-items-center">
                ...
                <?php if ($tag_nombre_actual): ?>
                    <span class="badge bg-info text-dark p-2 ms-2">
                        Tag: #<?= htmlspecialchars($tag_nombre_actual) ?>
                        <a href="tags.php" class="text-dark text-decoration-none ms-1 fw-bold" title="Quitar tag">✕</a>
                    </span>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="col-lg-4">
            <form action="videos_por_tag.php" method="GET" class="d-flex">
                <?php if ($active_tag_id): ?>
                    <input type="hidden" name="tag_id" value="<?= htmlspecialchars($active_tag_id) ?>">
                <?php endif; ?>
                <?php if ($active_tag): ?>
    ...
    <?php if (empty($videos)): ?>
        <div class="alert alert-secondary text-center my-5 bg-dark text-light border-secondary p-5 shadow">
            ...
            <p class="mb-0 text-secondary">Prueba ajustando la búsqueda, la fecha o selecciona otro tag.</p>
        </div>
    <?php else: ?>
